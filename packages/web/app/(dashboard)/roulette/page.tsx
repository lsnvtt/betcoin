...
export default function RoulettePage() {
  ...
  const addBet = useCallback((type: BetType, number?: number) => {
    ...
    setBets((prev) => {
      ...
      if (existing) {
        return prev.map((b) => b === existing ? { ...b, amount: b.amount + chipAmount } : b);
      }
  ...
  const spinWheel = useCallback(() => {
    ...
    if (totalBet > currentBalance) return;

    setSpinning(true);
    setResult(null);
    setLastPayout(null);

    adjustDemoBalance(-totalBet);
    setBalance(getDemoBalance());

    const winNumber = Math.floor(Math.random() * 37);
    // Calculate target position: we want winNumber centered under pointer
    // Strip is 5x37 numbers, each NUM_WIDTH wide. Land somewhere in the 3rd repetition
  ...
  }, [authenticated, login, spinning, bets, totalBet]);
  ...
  // Build number grid rows (standard roulette layout: 3 rows of 12)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6">
            <div className="relative overflow-hidden h-24 rounded-xl border border-white/10 bg-black/40">
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 w-0 h-0
                ...
                border-t-[14px] border-t-yellow-400" />
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 z-20 w-0 h-0
                border-l-[10px] border-l-transparent border-r-[10px] border-r-transparent
              ...
              <motion.div
                ...
                transition={spinning ? {
                  duration: 3.8,
                  ease: [0.15, 0.0, 0.05, 1.0],
                } : { duration: 0 }}
              >
                {STRIP_NUMBERS.map((n, i) => (
                  <div key={i} className={cn(
                    'flex-shrink-0 flex items-center justify-center h-16 rounded-lg mx-0.5 text-xl font-bold font-mono select-none',
                    getColorClasses(n),
                    result === n && !spinning && 'ring-2 ring-yellow-400'
                  )} style={{ width: NUM_WIDTH - 4 }}>
                    {n}
                  </div>
                ))}
              </motion.div>
            ...
            <AnimatePresence>
              {result !== null && !spinning && (
                ...
                >
                  <div className={cn(
                    'w-16 h-16 rounded-xl flex items-center justify-center text-2xl font-black font-mono',
                    getColorClasses(result)
                  )}>
                    {result}
                  </div>
                  <div className={cn(
                    'text-2xl font-bold font-mono',
                    lastPayout !== null && lastPayout >= 0 ? 'text-green-400' : 'text-red-400'
                  )}>
                    {lastPayout !== null && (lastPayout >= 0 ? `+${formatBetCoin(lastPayout)}` : formatBetCoin(lastPayout))}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6">
              ...
              {CHIP_VALUES.map((val) => (
                <button key={val} onClick={() => setChipAmount(val)} disabled={spinning}
                  className={cn(
                    ...
                    chipAmount === val
                      ? 'border-yellow-400 bg-yellow-400/20 text-yellow-400 scale-110 shadow-[0_0_12px_rgba(250,204,21,0.3)]'
            ...
            {/* Number grid - standard roulette table layout */}
              ...
              <div className="min-w-[600px]">
                <div className="grid gap-1" style={{ gridTemplateColumns: 'auto repeat(12, 1fr)' }}>
                  ...
                  <button onClick={() => addBet('number', 0)} disabled={spinning}
                    className={cn(
                      'row-span-3 rounded-lg bg-green-600/80 text-white text-lg font-bold hover:bg-green-600 transition-colors relative min-h-[108px]',
                      getBetAmount('number', 0) > 0 && 'ring-2 ring-yellow-400'
                    )}>
                    0
                    {getBetAmount('number', 0) > 0 && (
                      <span className="absolute bottom-1 right-1 bg-yellow-400 text-black text-[9px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
                  ...
                  {row1.map((n) => (
                    <button key={n} onClick={() => addBet('number', n)} disabled={spinning}
                      className={cn(
                        'rounded-lg p-2 text-sm font-bold transition-colors hover:brightness-125 relative h-[34px]',
                        getColorClasses(n),
                        getBetAmount('number', n) > 0 && 'ring-2 ring-yellow-400'
                      )}>
                      {n}
                      {getBetAmount('number', n) > 0 && (
                        <span className="absolute -top-1 -right-1 bg-yellow-400 text-black text-[8px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                ...
                <div className="grid grid-cols-3 gap-1 mt-1" style={{ marginLeft: 'calc(100% / 13 + 4px)', width: 'calc(100% - 100% / 13 - 4px)' }}>
                  {[
                    ...
                    <button key={bet.type} onClick={() => addBet(bet.type)} disabled={spinning}
                      className={cn(
                        'rounded-lg p-2 text-xs font-bold border border-white/20 bg-white/5 text-white hover:bg-white/10 transition-colors',
                        getBetAmount(bet.type) > 0 && 'ring-2 ring-yellow-400'
                      )}>
                      {bet.label}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-6 gap-1 mt-1" style={{ marginLeft: 'calc(100% / 13 + 4px)', width: 'calc(100% - 100% / 13 - 4px)' }}>
                  {[
                    ...
                    <button key={bet.type} onClick={() => addBet(bet.type)} disabled={spinning}
                      className={cn(
                        'rounded-lg p-2 text-xs font-bold border transition-colors hover:brightness-125',
                        bet.cls,
                        getBetAmount(bet.type) > 0 && 'ring-2 ring-yellow-400'
                      )}>
                      {bet.label}
            ...
            {bets.length > 0 && (
              <div className="bg-white/5 rounded-xl p-3 mt-4 border border-white/5">
                <p className="text-xs text-gray-400 mb-2">
                  Apostas: {bets.length} | Total: <span className="text-yellow-400 font-mono">{formatBetCoin(totalBet)}</span>
                </p>
            ...
            <div className="flex gap-3 mt-4">
              ...
              <Button onClick={clearBets} variant="outline" size="xl" disabled={spinning}>
                Limpar
              </Button>
            </div>
          </div>
