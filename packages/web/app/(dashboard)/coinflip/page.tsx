...
export default function CoinFlipPage() {
  ...
  const handleFlip = async () => {
    ...
    if (flipping || !walletAddress) return;
    ...
    try {
      const response = await startGame(walletAddress, 'coinflip', {
        betAmount,
        chosenSide: choice,
      });

      const flipResult = response.result as Choice;
      const won = flipResult === choice;
      const payout = response.payout || 0;
      const newBal = response.newBalance ?? balance;

      // Short animation delay before showing result
      ...
      setLastWon(won);
      setBalance(newBal);
      setBalanceDelta(won ? payout - betAmount : -betAmount);
      window.dispatchEvent(new Event('balance-updated'));

      setHistory((prev) => [
        { id: Date.now(), choice, result: flipResult, amount, won, sessionId: response.sessionId },
        ...prev.slice(0, 19),
      ]);
    } catch (err) {
      setFlipping(false);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-gray-400 mt-2">Escolha cara ou coroa e duplique sua aposta.</p>
      </motion.div>

      {error && (
        <div className="mb-4 p-3 rounded-xl bg-betcoin-red/10 border border-betcoin-red/20 text-betcoin-red-light text-sm">
          {error}
        </div>
      )}
            ...
            <CardContent>
                ...
                {lastWon !== null && !flipping && (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      lastWon ? 'bg-betcoin-accent/10 border-betcoin-accent/20' : 'bg-betcoin-red/10 border-betcoin-red/20'
                    )}
                  >
                    <p className={cn('text-2xl font-bold font-mono', lastWon ? 'text-betcoin-accent' : 'text-betcoin-red-light')}>
                      {lastWon ? `+${formatUSDT(parseFloat(amount) * 0.96)}` : `-${formatUSDT(amount)}`}
                    </p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {(['heads', 'tails'] as const).map((side) => (
                  <motion.button key={side} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    onClick={() => setChoice(side)} disabled={flipping}
                    className={cn(
                      'rounded-xl p-5 text-center transition-all border-2',
                      choice === side ? 'border-betcoin-primary bg-betcoin-primary/10 shadow-glow-orange' : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/20'
                    )}>
                    <span className={cn('text-3xl font-black block mb-1', choice === side ? 'text-betcoin-primary' : 'text-gray-500')}>
                      {side === 'heads' ? 'B' : 'C'}
                    </span>
                    <span className={cn('text-sm font-semibold', choice === side ? 'text-betcoin-primary' : 'text-gray-500')}>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex gap-2">
                  <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} min="1" step="1" disabled={flipping} placeholder="0.00" className="font-mono" />
                </div>
                <div className="flex gap-2 flex-wrap">
                  {quickAmounts.map((val) => (
                    <Button key={val} variant="outline" size="sm" onClick={() => setAmount(val)} disabled={flipping}
              ...
              <div className="flex justify-between items-center mb-6 px-1">
                ...
                <span className="text-lg font-bold font-mono text-white">{formatUSDT(payout)}</span>
              </div>

              {betAmount > balance && balance > 0 && (
                <p className="text-xs text-betcoin-red-light mb-2 text-center">Saldo insuficiente</p>
              )}
              <Button onClick={handleFlip} disabled={flipping || !amount || betAmount <= 0 || betAmount > balance}
                loading={flipping} size="xl" className="w-full text-lg font-bold">
                {flipping ? 'Jogando...' : authenticated ? 'Jogar CoinFlip' : 'Conectar para Jogar'}
              </Button>
            </CardContent>
          </Card>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {history.map((entry) => (
              <motion.div key={entry.id} initial={{ scale: 0 }} animate={{ scale: 1 }}
                className={cn('h-3 w-3 rounded-full', entry.won ? 'bg-betcoin-accent shadow-glow-green' : 'bg-betcoin-red shadow-glow-red')} />
            ))}
            ...
            {history.map((entry) => (
              <motion.div key={entry.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="flex items-center gap-2">
                  <div className={cn('h-2 w-2 rounded-full', entry.won ? 'bg-betcoin-accent' : 'bg-betcoin-red')} />
                  <span className="text-xs text-gray-400">{entry.choice === 'heads' ? 'Cara' : 'Coroa'}</span>
                </div>
                <span className={cn('text-xs font-mono font-semibold', entry.won ? 'text-betcoin-accent' : 'text-betcoin-red-light')}>
                  {entry.won ? `+${formatUSDT(parseFloat(entry.amount) * 0.96)}` : `-${formatUSDT(entry.amount)}`}
                </span>
              </motion.div>
            ))}
          </div>
